...
version 1.1.0, there are methods for factor and numeric x-coordinate data and
...
A scatter plot matrix shows multiple pairs of x and y data in separate graphs. This example plots some data in the IonBalance dataset to assess some of the relations among selected cations. The function \texttt{setSplom} must be used to set up the graphics device. For many pairs of data, a very large graphics device can be created. The \texttt{Panel} argument is a very flexible way to add statistics or selected lines to each plot.
<<echo=TRUE>>=
# setSweave is a specialized function that sets up the graphics page for
# Sweave scripts. It should be replaced by a call to setPage or setPDF 
# in a regular script.
...
\textbf{Figure 9.} A scatter plot matrix with an explanation.
