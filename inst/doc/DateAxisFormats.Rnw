...
The \texttt{timePlot} function controls the formatting of the date- (x-) axis with the \texttt{xlabels} argument. The default value for \texttt{xlabels} is "Auto" but may be any of "hours," "days," "months," "years," or "water years" that will select a specific format. The value for the \texttt{xlabels} argument may also be a list of the arguments to the \texttt{detePretty} function for even more control. The examples in this vignette use primarily the "Auto" option, but at least one example shows one of the other options.
