...
`Box` argument. The simple box plot extends the whiskers to the minimum
and maximum values. For the Tukey box plot, `type="tukey"`, the whiskers
...
plot. The explaantion for the Tukey box plot requires a width of about 2.5
...
The Box argument includes the show.counts components, which is a logical value (`TRUE` or `FALSE`) that controls whethere the number of
...
where individual values are plotted rather than a box; width, which is a single numeric value that specifies the width of the box in inches; fill, which specifes the color of the box; and truncated, which specify the limits for the truncated and extended types of boxplot.
