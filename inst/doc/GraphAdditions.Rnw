...
Occasionally there is a need to show the general area coverd by a set of data. The \texttt{smwrGraphs} package has several functions that help the user draw areas covered by the data: \texttt{dataEllipse}, \texttt{cov2Ellipse}, and \texttt{hull} functions will provide closed polygons that represent some measure of the area covered by data. This example demonstrates \texttt{dataEllipse} and \texttt{hull}.

This exmaple also identifies and labels selected points lying outside of the enclosing polygon. It demonstrates the use of the \texttt{lablePoints} function.
